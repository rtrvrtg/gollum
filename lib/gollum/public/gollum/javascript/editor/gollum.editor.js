...
(function($) {
   ...
   *  TextManipulator
   *
   *  Manipulate content in the editor's textarea.
   *  Used to help with FunctionBar behaviours.
   *  Loads when DOM is ready.
   */

  $(document).ready(function(){
    $('#gollum-editor-body').textManipulator();
  });
   ...
   */
   var FunctionBar = {
      ...
      executeAction: function( definitionObject ) {
        ...
        if ( definitionObject.manipulate && definitionObject.manipulate.length > 0 ) {
          var parts = ['gollum-editor-body'].concat(definitionObject.manipulate);
          $.textManipulator.apply(null, parts);
          return;
        }
        ...
        $('#gollum-editor-body').trigger('change');
  ...
  $.GollumEditor.manipulate = function() {
    var args = Array.prototype.slice.call(arguments);

    if (args.length > 0) {
      var parts = ['gollum-editor-body'].concat(args);
      $.textManipulator.apply(null, parts);
      return;
    }
  };
