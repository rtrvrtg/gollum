...
(function($) {
  ...
  var Dialog = {
    ...
    attachEvents: function( evtOK ) {
      ...
      $('#gollum-dialog-dialog-inner').find("input.url").keyup(function( e ){
        Dialog.eventUrlKeyup( e, evtOK );
      });
    },
    ...
    detachEvents: function() {
      ...
      $('#gollum-dialog-dialog-inner').find("input.url").unbind('keyup');
    },
    ...
    urlAutocompleteCache: {},

    urlAutocompleteSend: function( target, value ) {

      var filterAndSend = function(jsonData){
        var filteredData = jsonData.filter(function(item){
          return item.path.toLowerCase().indexOf(value.toLowerCase()) == 0;
        });

        console.log(filteredData.length);

        Dialog.urlAutocompleteShow(target, filteredData);
      };

      var parts = value.split("/");
      var currentSearch = parts.pop();
      ...
      if (Dialog.urlAutocompleteCache[url]) {
        filterAndSend(Dialog.urlAutocompleteCache[url]);
      }
      else {
        $.ajax({
          url: url,
          dataType: "json"
        }).done(function(jsonData, jqXHR, textStatus){
          Dialog.urlAutocompleteCache[url] = jsonData;
          filterAndSend(Dialog.urlAutocompleteCache[url]);
        });
      }
    ...
    urlAutocompleteShow: function( target, results ) {
      if (!results || results.length == 0) {
        $('.gollum-dialog-autocomplete-selection').remove();
      }
      else {
        $('.gollum-dialog-autocomplete-selection').find('li').remove();

        var itemList = $('.gollum-dialog-autocomplete-selection');

        if (!$('.gollum-dialog-autocomplete-selection').length) {
          itemList = $('<ul />', {
            'class': 'gollum-dialog-autocomplete-selection'
          });
        }
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          item.bind('mouseup touchend', function(e){
            e.preventDefault();
            target.value = $(this).text().replace(/^\s*/, '').replace(/\s*$/, '');
            Dialog.urlAutocompleteShow(target, []);
          });

          itemList.append(item);
        }

        itemList.insertAfter(target);
      }
    },

    eventUrlKeyup: function( e, evtOK ){
      var value = e.target.value;

      if (value.length == 0) {
        Dialog.urlAutocompleteShow(e.target);
        return;
      }

      var prefixes = ["http://", "https://", "mailto:", "data:", "tel:", "geo:"];

      var isCompleteURL = (prefixes.map(function(prefix){
        return value.indexOf(prefix) == 0 ? 1 : 0;
      ...
      Dialog.urlAutocompleteSend(e.target, value);
    },
