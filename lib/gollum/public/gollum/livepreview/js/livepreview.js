...
// Dummy object to help capture Gollum's help text
...
$.GollumEditor = {
  ...
  defineLanguage: function(name, data) {
    this.data.langs[name] = data;
  },
  defineHelp: function(name, data) {
    this.data.helps[name] = data;
  },
  getHelpHtml: function(name) {
    var parts = [];
    for (var i = 0; i < this.data.helps[name].length; i++) {
      var part = this.data.helps[name][i], partChunks = [];
      partChunks.push('<h2>' + part.menuName + '</h2>');
      for (var j = 0; j < part.content.length; j++) {
        partChunks.push('<h3>' + part.content[j].menuName + '</h3>');
        partChunks.push(part.content[j].data + '<br />');
      }
      parts.push(partChunks.join(""))
    }
    return parts.join("");
  }
};
...
var editorPreview = {
  languages: [ 'c', 'c++', 'cpp', 'clojure', 'coffee',
  ...
  setupLanguages: function(){
    var languagesLength = editorPreview.languages.length;
    for ( var a = 0; a < languagesLength; a++ ) {
      var name = editorPreview.languages[a];
      editorPreview.langModes[name] = false;
    }
  },

  loadLanguage: function(language){
    var mode = editorPreview.langModes[language];
  ...
  highlightMarkup: function(element, language) {
    ...
    data = data.trim();
    var mode = editorPreview.loadLanguage( language );
  ...
  pygmentsLanguageToAceMode: function(declaredLanguage) {
  ...
  getHTML: function(forceRedraw){
    var text = editorSession.getValue();

    if (text == undefined || text == '') {
      editorPreview.setPreview('');
      return;
    }
    ...
    else if (text && text == oldInputText) {
    ...
    // Try and get some fragments.
    // We'll alter the preview text along the way with any pre-cached fragments.
    var fragments = editorPreview.identifyFragments(text);

    if (fragments.length > 0) {
      var cacheFragResult = editorPreview.replaceCachedFragments(text, fragments);
      // No tuples mo problems
      text = cacheFragResult[0];
      var uncachedFragments = cacheFragResult[1];

      if (uncachedFragments.length > 0) {
        editorPreview.fetchMoreFragments(uncachedFragments);
      }
    }
    ...
    if (codeElementsLength > 0) {
      for (var idx = 0; idx < codeElementsLength; idx++) {
        ...
        if (txt.length > 1 && codeHTML.match(/\n/) ) {
          ...
          var aceMode = editorPreview.pygmentsLanguageToAceMode(declaredLanguage);

          if ($.inArray(declaredLanguage, editorPreview.languages) === -1) {
            // Unsupported language.
            skipped++;
            continue;
          }
          // highlight: element
          editorPreview.highlightMarkup(element, aceMode);
        } else {
  ...
  fetchMoreFragments: function(fragments){
    $.ajax({
      url: "../fragments",
      type: "POST",
      accepts: "json",
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        editorPreview.cachedFragments[source] = destination;
      }

      editorPreview.getHTML(true);
    });
  },

  identifyFragments: function(text){
    var fragments = [];
  ...
  replaceCachedFragments: function(text, fragments) {
    var uncachedFragments = [];

    for (var i = 0; i < fragments.length; i++) {
      var frag = fragments[i];
      if (editorPreview.cachedFragments[frag]) {
        text = text.replace(frag, editorPreview.cachedFragments[frag], "g");
      }
      else {
  ...
  setPreview: function(markup) {
    if (editorPreview.chosenPreview) {
      editorPreview.chosenPreview(markup);
      return;
    }

    try {
      editorPreview.optimalPreview(markup);
      chosenPreview = editorPreview.optimalPreview;
    } catch ( e ) {
      editorPreview.fallbackPreview(markup);
      chosenPreview = editorPreview.fallbackPreview;
    }
  },
...
editorPreview.setupLanguages();

var makePreviewHtml = function () {
  return editorPreview.getHTML();
};
      ...
      $.magnificPopup.open({
        ...
        callbacks: {
          ...
          open: function(){
            ...
            if (!form.data('has-form-actions')) {
              $("#new-link-url").select2({
                ...
                if (e.added.id.indexOf("internal_") === 0) {
                  $('#new-link-destination').val('internal');
                }
                else if (e.added.id.indexOf("external_") === 0) {
                  $('#new-link-destination').val('external');
                }
                else {
                  $('#new-link-destination').val(''); 
                }
              });

              var makeLink = function(){

                var url = $('#new-link-url').val();
                var isInternal = url.indexOf('internal_') === 0;
                url = url.replace(/^(internal|external)_/, '');

                var title = $('#new-link-title').val();

                if (isInternal) {
                  if (title.length > 0) {
                    editorUtilities.insert('[[' + title + '|' + url + ']]', true);
                  }
                  else {
                    editorUtilities.insert('[[' + url + ']]', true);
                  }
                }
                else {
                  if (title.length > 0) {
                    ...
                    editorUtilities.insert('[' + url + '](' + url + ')', true);
                  }
                }
                return true;
              };
          ...
          close: function() {
            $('#new-link-title').val("");
            $('#new-link-url').val("");
            $('#new-link-url').select2("data", null);
          }
        }
      }, 0);
    },
          ...
          open: function(){
            ...
            if (!form.data('has-form-actions')) {
              var makeImage = function(){
                editorUtilities.insert('![' + $('#new-image-alt').val() + '](' + $('#new-image-url').val() + ')', true);
                return true;
              };
      ...
      $.magnificPopup.open({
        ...
        callbacks: {
          ...
          open: function(){
            ...
            // Populate the page
            require(["../javascript/editor/langs/markdown"], function(util) {
              page.find('#help-page-content').html($.GollumEditor.getHelpHtml('markdown'));
            });
          }
        }
      }, 0);
